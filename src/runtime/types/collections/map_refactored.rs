...
pub struct MapTypeRefactored;

impl MapTypeRefactored {
    /// Returns complete type metadata with all methods
    pub fn metadata() -> TypeMetadata {
        TypeMetadata::new("map", "Key-value map/dictionary type")
            .with_instance_methods(vec![
                ...
                MethodMetadata::new("set", "null", "Set value for key")
                    .with_params(vec![
                ...
                MethodMetadata::new("has", "bool", "Check if key exists")
                    .with_params(vec![ParamMetadata::new("key", "any")]),
                MethodMetadata::new("delete", "bool", "Delete key, returns true if existed")
                    .with_params(vec![ParamMetadata::new("key", "any")])
                    .with_alias("remove"),
                MethodMetadata::new("clear", "null", "Remove all entries"),
                MethodMetadata::new("size", "int", "Get number of entries")
                    .with_alias("length"),
...
#[async_trait]
impl TypeHandler for MapTypeRefactored {
...
mod tests {
    ...
    #[test]
    fn test_map_get_set() {
        let handler = MapTypeRefactored;
        let mut value = RuntimeValue::Map(MapValue {
            entries: HashMap::new(),
            key_type: crate::ast::types::PrimitiveType::any(),
            value_type: crate::ast::types::PrimitiveType::any(),
        });
        ...
        handler
            .call_instance_method(
                ...
                "set",
                vec![
                    RuntimeValue::Str(StrValue::new("key1".to_string())),
        ...
        let result = handler
            .call_instance_method(
                &mut value,
                "get",
                vec![RuntimeValue::Str(StrValue::new("key1".to_string()))],
                Position::default(),
                None,
            )
            .unwrap();
    ...
    #[test]
    fn test_metadata() {
        let metadata = MapTypeRefactored::metadata();
        assert_eq!(metadata.type_name, "map");
        assert!(metadata.has_instance_method("get"));
        assert!(metadata.has_instance_method("set"));
        assert!(metadata.has_instance_method("delete"));
        assert!(metadata.has_instance_method("remove")); // alias
    }
}
